    ...
    public String randomText() {
        ...
        for (int i = 0; i < OUTPUT_WORD_COUNT; i++) {
            ...
            if (ngrams.containsKey(currentNGram)){
                ArrayList<String> wordList = ngrams.get(currentNGram);
                Random rand = new Random(wordList.size());
                int r = rand.nextInt();
                String myWord = wordList.get(r);
                stringWriter.append(myWord + " ");
            } else {
                LinkedList<String> randGram = randomNGram();
                ArrayList<String> wordList = ngrams.get(randGram);
                if (wordList.size() > 1) {
                    Random rand = new Random(wordList.size() -1);
                    int r = random.nextInt(wordList.size());
                    // works but only with first word from array list in value
                    String myWord = wordList.get(r);
                    stringWriter.append(myWord + " ");
                } else {
                    String myWord = wordList.get(0);
                    stringWriter.append(myWord + " ");
                }
            }
            //
